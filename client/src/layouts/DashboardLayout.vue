  ...
  <div class="d-flex" style="min-height: 100vh; padding-top: 60px;">
    <aside class="dashboard-sidebar" :class="{ collapsed: collapsed }">
      <div class="p-3 border-bottom text-center" style="border-color:var(--glass-border)!important;">
        ...
        <button class="btn btn-sm btn-outline-light" @click="collapsed = !collapsed" style="font-size:0.8rem;"><i class="bi bi-list"></i></button>
      </div>
    ...
    <div class="flex-grow-1 p-4" style="background:var(--dark);min-height:100vh;overflow-y:auto;">
      <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex align-items-center gap-3">
          <router-link to="/dashboard/notifications" class="position-relative text-light" style="font-size:1.2rem;">
            <i class="bi bi-bell"></i>
            <span v-if="notifStore.unreadCount" class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="font-size:0.55rem;">{{ notifStore.unreadCount }}</span>
          </router-link>
        </div>
      </div>
      <router-view />
...
<style scoped>
.dashboard-sidebar { width: 220px; background: var(--surface); border-right: 1px solid var(--glass-border); transition: width .3s; flex-shrink: 0; overflow: hidden; }
